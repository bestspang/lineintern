      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold">
            {locale === 'th' ? '🏃 อนุมัติขอกลับก่อน' : '🏃 Approve Early Leave'}
          </h1>
        ...
        <Card className="border-dashed">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground">
              {locale === 'th' ? 'ไม่มีคำขอกลับก่อนที่รออนุมัติ' : 'No pending early leave requests'}
            </p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={req.id} className="overflow-hidden">
              <CardContent className="p-4">
                ...
                <div className="bg-muted/50 rounded-lg p-3 mb-3">
                  ...
                  <p className="text-sm text-muted-foreground mb-2">{req.leave_reason}</p>
                  {req.actual_work_hours !== null && req.required_work_hours !== null && (
                    <div className="text-xs text-muted-foreground">
                      {locale === 'th' ? 'ทำงานแล้ว' : 'Worked'}: {req.actual_work_hours.toFixed(1)}/{req.required_work_hours.toFixed(1)} {locale === 'th' ? 'ชม.' : 'hrs'}
                    </div>
                  )}
                </div>
